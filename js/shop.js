(function () {
  const apiBase = '/api/shop';
  const cartKey = 'dala-shop-cart';
  let toastTimer;
  ...
  function productCard(product) {
    return `
      <article class="shop-product-card">
        <a class="shop-product-card__media" href="/shop/product.html?slug=${product.slug}">
  ...
  function categoryCard(category) {
    return `
      <article class="shop-category-card">
        <a class="shop-category-card__media" href="/shop/category.html?slug=${category.slug}">
  ...
  async function initShopHome() {
    const [categoryResult, featuredResult] = await Promise.all([
      api('/categories'),
    ...
    if (collectionTabs) {
      collectionTabs.innerHTML = categoryResult.categories
        .slice(0, 6)
        .map((category) => `<a class="shop-tab" href="/shop/category.html?slug=${category.slug}">${category.name}</a>`)
        .join('');
    }

    if (categoryGrid) {
      categoryGrid.innerHTML = categoryResult.categories.map(categoryCard).join('');
    }
  ...
  async function initCategoryPage() {
    ...
    if (tabs) {
      tabs.innerHTML = categoryResult.categories
        .map((item) => `<a class="shop-tab ${item.slug === category.slug ? 'is-active' : ''}" href="/shop/category.html?slug=${item.slug}">${item.name}</a>`)
        .join('');
    }
    ...
    if (headerImage) {
      headerImage.src = category.heroImage;
  ...
  async function initProductPage() {
    ...
    if (featureList) {
      featureList.innerHTML = [
        'Retail-ready presentation for premium shelves',
        'Live pricing and manual order checkout',
        'Built for category-first browsing and repeat purchase'
    ...
    if (quoteButton) {
      quoteButton.addEventListener('click', () => {
        addToCart(payload, 1);
        window.location.href = '/shop/checkout.html?mode=quote';
      });
    }
  }
    ...
    if (!items.length) {
      container.innerHTML = `
        <div class="shop-empty">
          ...
          <a class="shop-btn shop-btn-primary" href="/shop/">Browse the shop</a>
        </div>
      `;
  ...
  function initCheckoutPage() {
    ...
    if (!items.length) {
      form.innerHTML = `
        <div class="shop-empty">
          ...
          <a class="shop-btn shop-btn-primary" href="/shop/">Back to the shop</a>
        </div>
      `;
      return;
    }
    ...
    form.addEventListener('submit', async (event) => {
      ...
      try {
        ...
        setCart([]);
        window.location.href = `/shop/order-success.html?order=${encodeURIComponent(result.orderNumber)}&mode=${mode}`;
      } catch (error) {
        toast(error.message);
      } finally {
